  ...
  return (
    <form onSubmit={handleSubmit} className="px-80 m-auto p-8 flex flex-col gap-y-4">
      ...
      <div className="flex flex-col gap-y-2">
        ...
        <input type="text" className="border border-secondary-color p-2 rounded-lg" value={role} onChange={(e) => setRole(e.target.value)} />
      </div>
      <div className="flex flex-col gap-y-2">
        <label htmlFor="">Kata Sandi</label>
        <input type="password" className="border border-secondary-color p-2 rounded-lg" value={password} onChange={(e) => setPassword(e.target.value)} />
      </div>
